...
opencv_detector = build_model()


def detect_face(img, mode='eyes'):
    try:
        faces = opencv_detector["face_detector"].detectMultiScale(img, 1.1, 10)
    ...
    if len(faces) > 0:
        ...
        if mode == 'eyes':
            eyes = detect_eyes(opencv_detector["eye_detector"], detected_face)

            if eyes is None:
                return None
            else:
                (lx, ly), (rx, ry) = eyes
                eyes = (lx + x, ly + y), (rx + x, ry + y)
                return eyes
        elif mode == 'box':
            return x, y, w, h
        else:
...
def detect_eyes(eye_detector, img):
